...
function getTemplateParameters(
  ...
  if (source) {
    ...
    templateParameters.functionName = `${invocationCallCode})`
    templateParameters.location = callLocation

    if (!isPanic) {
      source = source.mapLineAt(callLocation.lineNumber, (line) => {
        const slicePoint = invocationLine.indexOf('{')
        return slicePoint === -1 ? line : line.slice(0, slicePoint)
      })
    }
...
function findPrismaActionCall(str: string): string | null {
